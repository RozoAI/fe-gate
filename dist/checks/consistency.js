...
export function uncoveredPages(appDir, specDir, ignore = []) {
    ...
    for (const p of pages) {
        ...
        if (!candidates.some(existsSync))
            missing.push(`/${route}  (expected ${relative(process.cwd(), candidates[0])})`);
    }
    return missing;
}
/**
 ...
 */
export function i18nParity(localesDir, reference = "en") {
    const files = readdirSync(localesDir).filter((f) => f.endsWith(".json"));
    const load = (f) => flatten(JSON.parse(readFileSync(join(localesDir, f), "utf8")));
    const refFile = files.find((f) => f === `${reference}.json`);
    if (!refFile)
        return [`reference locale ${reference}.json not found in ${localesDir}`];
    ...
    for (const f of files) {
        ...
        for (const k of Object.keys(ref))
            if (!(k in loc))
                problems.push(`${f}: missing key ${k}`);
        for (const k of Object.keys(loc))
            if (!(k in ref))
                problems.push(`${f}: extra key ${k}`);
        for (const k of Object.keys(ref))
            if (k in loc && loc[k] === ref[k] && String(ref[k]).length > 12 && f !== "en.json")
 ...
 */
export function vercelRewriteCoverage(keys, vercelJsonPath, pattern = (slug) => `/services/${slug}`) {
    const v = JSON.parse(readFileSync(vercelJsonPath, "utf8"));
    const sources = [...(v.rewrites ?? []), ...(v.redirects ?? [])].map((r) => r.source);
...
export function scanBuiltHtml(outDir) {
    const problems = [];
    for (const f of walk(outDir, (p) => p.endsWith(".html"))) {
        const html = readFileSync(f, "utf8");
        const rel = relative(process.cwd(), f);
        if (/href=""/.test(html))
            problems.push(`${rel}: empty href`);
        if (/\w<\/strong>\w/.test(html))
            problems.push(`${rel}: missing space after </strong> (Next 16 trap)`);
        if (!/property="og:image"/.test(html) && !/name="robots"[^>]*noindex/.test(html))
...
export function forbiddenPhrases(rootDir, phrases, exts = [".ts", ".tsx", ".js", ".jsx", ".json", ".md", ".html"]) {
    const hits = [];
    for (const f of walk(rootDir, (p) => exts.some((e) => p.endsWith(e)))) {
        const txt = readFileSync(f, "utf8");
        for (const ph of phrases)
            if (txt.includes(ph))
                hits.push(`${relative(rootDir, f)}: "${ph}"`);
    }
    return hits;
}

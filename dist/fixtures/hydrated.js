 ...
 */
export async function expectHydrated(page, box, opts = {}) {
    await page.waitForLoadState("networkidle").catch(() => { });
    // Next.js marks hydration completion by attaching React; give the client a beat.
    await page.waitForTimeout(300);
    const allowed = opts.allowErrors ?? [];
    const isAllowed = (e) => allowed.some((a) => (a instanceof RegExp ? a.test(e) : e.includes(a)));
    ...
    if (opts.title)
        await expect(page).toHaveTitle(opts.title instanceof RegExp ? opts.title : new RegExp(escape(opts.title)));
    if (opts.h1)
        await expect(page.locator("h1").first()).toContainText(opts.h1);
    if (opts.pathname) {
        const p = new URL(page.url()).pathname;
        if (opts.pathname instanceof RegExp)
            expect(p).toMatch(opts.pathname);
        else
            expect(p).toBe(opts.pathname);
    }
}
/** Every anchor must have a non-empty href — the silent `href=""` class (rozo-landing PR #23). */
export async function expectNoEmptyHref(page) {
    const bad = await page.$$eval("a", (as) => as.filter((a) => !a.getAttribute("href") || a.getAttribute("href") === "#").map((a) => a.outerHTML.slice(0, 120)));
    expect(bad, `anchors with empty href:\n${bad.join("\n")}`).toEqual([]);
}
/** Primary CTA is visible and links somewhere real. */
export async function expectPrimaryCta(page, selector) {
    const cta = page.locator(selector).first();
    await expect(cta).toBeVisible();
    const href = await cta.getAttribute("href");
    if (href !== null)
        expect(href.trim().length, `CTA ${selector} has empty href`).toBeGreaterThan(0);
}
/** A QR code that rendered but is blank (Stellar C-address incident) — assert non-empty pixels. */
export async function expectQrNonEmpty(page, selector = "canvas, svg[data-qr], img[alt*='QR' i]") {
    ...
    const nonEmpty = await el.evaluate((node) => {
        ...
        if (node instanceof SVGElement)
            return node.querySelectorAll("rect, path").length > 20;
        if (node instanceof HTMLImageElement)
            return node.naturalWidth > 0 && !!node.src;
        return false;
    });
    expect(nonEmpty, "QR rendered but empty").toBe(true);
}
/** Text that must NOT appear (removed copy, stale flag copy, brand leak). */
export async function expectBodyExcludes(page, phrases) {
    const body = await page.locator("body").innerText();
    for (const p of phrases) {
        if (p instanceof RegExp)
            expect(body).not.toMatch(p);
        else
            expect(body, `forbidden phrase present: ${p}`).not.toContain(p);
    }
}
function escape(s) {
    return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

...
2. 老板批准后应用 rozo-intents-api migration，再按该仓 runbook 启用 pg_cron。绝不把生产 DB 连接串放进 GitHub。
...
真钱只允许 main 上第一次 attempt，不允许直接 rerun。先写订单标识，再创建和支付；create/broadcast 没有自动重试。任一失败或取消会阻止后续运行，不能因为跨日而自行再付款。下载 `real-money-evidence` artifact，按 `fe-gate-UTC日期-腿号` 查询订单并核对链上余额和 tx。确认结果后，老板可将 `FE_GATE_RECONCILED_RUN_ID` 设置为已核实的失败 run id，再新建一次运行；不要给未核实的 run 签收。
